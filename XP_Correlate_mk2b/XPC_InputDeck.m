...
try
  ...
  fprintf([' ' MTEXversion '  ']);
    ...
    try 
        addpath /Users/lukemulholland/Documents/MATLAB/mtex-5.0.3
    catch
        addpath(genpath('/Users/lukemulholland/Documents/MATLAB/mtex-5.0.3'))
    end
    startup_mtex
end
...
filepath    = '/Users/lukemulholland/Documents/MATLAB/XPCorrelate/Results_22_3_25/';  %Location of files
expressname = 'Pedro_Reformatted_05.03.25._Express_results.mat';
ebsdname    = 'A-Cr1_H Specimen 2 Site 1 Map Data 2.ctf'; %name of the ebsd with extension

epmaq       = 1; %epma analysis? 1=yes, 0=no
epmaname    = 'EDS_Mo.tiff'; %name of epma file if needed 
epmabsename = 'SE_450X_b.tiff'; %name of epma BSE file if needed 

gdcalcq     = 0; %grain boundary analysis? 1=yes, 0=no
...
EBSDREFq    = 1;%0 if just one phase: uses IPF for EBSD reference selection
                %1 if multiphase: uses phase map
                %2 if you want to use the pattern quality map
EPMArefq    = 0;%0 uses the chemical map for referencing
...
resultsdirold = strrep(resultsdirold, '/', '\');
...
%% 9 Save things
close all
save([fullfile(resultsdir,[filename(1:length(filename)-4) '_XPCorrelate_results_Many_Points' currdate]) '.mat']);
